...
export class ViewcommandesComponent implements OnInit{
  ...
  getDeliveryByAddress(address: string): Map<number, string> {
    let deliveryPeople: Map<number, string> = new Map<number, string>();
    for (const person of this.delivery_persons) {
        if (person.town === address) {
            deliveryPeople.set(person.id, person.name);
        }
    }
